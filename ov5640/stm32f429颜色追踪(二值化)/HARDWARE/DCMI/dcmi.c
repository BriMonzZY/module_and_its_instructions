...
void HAL_DCMI_FrameEventCallback(DCMI_HandleTypeDef *hdcmi)
{
	...
	for(i = 0; i < lcddev.width/2; i++) {
		for(j = 0; j < lcddev.height/4; j++) {
			color_lcd = LCD_ReadPoint(i, j);
			Image[j][i] = RGB565_To_Gray(color_lcd);
			//if(color_lcd >= yuzhi) LCD_Fast_DrawPoint(i, j+lcddev.height/4, WHITE);
			//else LCD_Fast_DrawPoint(i, j+lcddev.height/4, BLACK);
		}
	}
	GetHistGram(Image);
	Threshold_OTSU = OSTUThreshold();  /* 求出自适应阈值 */
	//printf("Threshold_OTSU : %d \n", Threshold_OTSU);
	for(i = 0; i < lcddev.width/2; i++) {
		for(j = 0; j < lcddev.height/4; j++) {
			if(Image[j][i] <= Threshold_OTSU) LCD_Fast_DrawPoint(i, j+lcddev.height/4, WHITE);
			else LCD_Fast_DrawPoint(i, j+lcddev.height/4, BLACK);
		}
	}
