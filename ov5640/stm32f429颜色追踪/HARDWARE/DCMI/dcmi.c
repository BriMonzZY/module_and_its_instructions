...
RESULT Resu;
TARGET_CONDI Condition = {0, 15, 90, 240, 75, 160, 30, 30, 320, 240};
...
void HAL_DCMI_FrameEventCallback(DCMI_HandleTypeDef *hdcmi)
{
	...
	if(Trace(&Condition,&Resu)) {
		...
		//POINT_COLOR=WHITE; 
		//LCD_DrawRectangle(Resu.x - Resu.w / 2, Resu.y - Resu.h / 2, Resu.w, Resu.h);
		LCD_DrawRectangle(Resu.x-Resu.w/2, Resu.y-Resu.h/2, Resu.w+Resu.w/2, Resu.h+Resu.h/2);
