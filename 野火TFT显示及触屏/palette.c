...
void Touch_Button_Init_2(void) {
  ...
  button[1].para = CL_BLUE;
  button[1].touch_flag = 0;  
  button[1].draw_btn = Draw_Color_Button ;
  button[1].btn_command = Draw_rec;
	
	button[2].start_x = BUTTON_START_X;
  button[2].start_y = COLOR_BLOCK_HEIGHT*7;
  button[2].end_x = BUTTON_START_X+COLOR_BLOCK_WIDTH ;
  button[2].end_y = LCD_Y_LENGTH;
  button[2].para = CL_BUTTON_GREY;
  button[2].touch_flag = 0;  
  button[2].draw_btn = Draw_Clear_Button ;    // 590行
  button[2].btn_command = Command_Clear_Palette ;
}


void Palette_Init_2(uint8_t LCD_Mode) {
	uint8_t i;
...
static void Draw_rec(void *btn)
{
  ...
  brush.color = ptr->para;	
	...
	LCD_SetColors(brush.color,brush.color);

	ILI9341_DrawRectangle(100, 100, 100, 100, 1);  // 全填充正方形
}
